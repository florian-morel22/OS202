...
if __name__ == '__main__':
    ...
    for k in range(100):
        ...
        if rank == 0:
            ...
            ### ---- receive diff from other processes ---- ###

            diff = []

            for k in range(1, size):
                diffk = comm.recv(source=k)
                diffk = [x + ((k-1)*div-1)*nx for x in diffk]
            ...
            diff = [x for x in diff if x//nx != 0 and x//nx != partial_grid.dimensions[0] - 1]
    ...
    if rank == 0:
        t2 = time.time()
        print(f"Temps d'execution par itération: {(t2-t1)/100} secondes")
