...
for r in range(nbre_repet):
    ...
    t1 = time.time()
    nuagek = nuage[rank*(nuage.shape[0]//size)+1:(rank+1)*(nuage.shape[0]//size),:] # on ne garde que la moitié du nuage
    enveloppe = calcul_enveloppe(nuagek)


    rank_bin = bin(rank)[2:].zfill(2) # Le 2 est à modifier pour généraliser
    for dim in range(len(rank_bin)-1, -1, -1):
        target = rank_bin[:dim] + str((int(rank_bin[dim])+1)%2) + rank_bin[dim+1:]
        target = int(target, 2) # Le 2 est à modifier pour généraliser

        comm.isend(enveloppe, dest=target, tag=dim)
        enveloppeTarget = comm.recv(source=target, tag=dim)

        enveloppe = np.concatenate((enveloppe, enveloppeTarget), axis=0)
