...
configs_by_proc = nombre_cas // size

k = 0

if rank != size - 1:

    for num_config in range(configs_by_proc * rank, configs_by_proc * (rank + 1)):
        k += 1
        t1 = time.time()
    ...
    for num_config in range(configs_by_proc * rank, nombre_cas):
        k += 1
        t1 = time.time()
        cells = np.zeros((nb_iterations, nb_cellules+2), dtype=np.int16)
        cells[0, (nb_cellules+2)//2] = 1
...
print(f"Processus {rank} a calcule {k} configurations.")
